...
public class ShootBehaviour : GenericBehaviour
{
	...
	public Texture2D aimCrosshair, shootCrosshair, crosshair_red;                 // Crosshair textures for aiming and shooting.
	...
	private void Update()
	{
		...
		else if (Input.GetKeyDown(KeyCode.G) || CrossPlatformInputManager.GetButtonDown("drop") && activeWeapon > 0)
		{
		...
		{
			if ((Input.GetKeyDown(KeyCode.H) || CrossPlatformInputManager.GetButtonDown("change") && !isChangingWeapon))
			{
				isChangingWeapon = true;
				int nextWeapon = activeWeapon + 1;
				ChangeWeapon(activeWeapon, (nextWeapon) % weapons.Count);
			}
			else if (Input.GetKeyDown(KeyCode.R) || CrossPlatformInputManager.GetButtonDown("reload"))
			{
			...
			if (shotDecay <= 0.4f)
			{
				...
				if (activeWeapon > 0)
				{
					...
					if (shotDecay <= (0.4f - 2 * Time.deltaTime))
					{
						...
						if (weapons[activeWeapon].mode == InteractiveWeapon.WeaponMode.AUTO && Input.GetMouseButton(0) || CrossPlatformInputManager.GetButton("shoot"))
						{
							ShootWeapon(activeWeapon);
						}
